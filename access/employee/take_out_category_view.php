...
<style>
body {
  /*background: linear-gradient(-45deg, #fbb034 0%, #ffdd00, #fbd72b, #f9484a);
...
<div class="row" id="body-row">
    ...
    <div id="sidebar-container" class="sidebar-expanded d-none d-md-block side">
        ...
        <ul class="list-group">
            ...
            <a href="take_out?t=Take-out" class="bg-light list-group-item list-group-item-action  active">
                <div class="d-flex w-100 justify-content-start align-items-center">
    ...
    <div class="col p-4">    
   <?php
if (isset($_GET["t"]) && isset($_GET["category"])) {
    $t_id = $_GET["t"];   
    $category_id = $_GET["category"]; 
   $user->table_number = $t_id;
      $stmt= $user->transfer_checker();
...
<div class="form-row">
  ...
  <a style="font-size: 0.9rem;" href="menu_view?table=<?php echo $table_id;?>" class="navbar-brand"></a>
    <a style="color: black;" href="my_order_take_out?table=<?php echo $table_id;?>" class="btn btn-warning my-2 my-sm-0" type="submit"><i class="fas fa-shopping-cart"></i> My Order</a>
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
       <li class="nav-item active" style="margin-right: 5px;">
        <a class="nav-link btn btn-outline-danger " onclick="goBack()"><i class="fas fa-arrow-left"></i>Back</a>
      </li>
      <li class="nav-item active" style="margin-right: 5px;">
        <a class="nav-link btn btn-warning " href="take_out_category?t=<?php echo $t_id;?>">Category View</a>
      </li>
      <li class="nav-item ">
        <a class="nav-link btn btn-outline-warning " href="take_out?t=<?php echo $t_id;?>">All Menu View</a>
      </li>
    </ul>
    <?php
   
    echo "
    <form class='form-inline my-2 my-lg-0' action='take_out_category_view?t=Take-out&category={$category_id}' method='POST'>";
...
<hr>
          ...
          <?php 
           ...
           $user->category_id = $_GET["category"]; 
      $stmt2= $user->view_menu_all_category_select_take_out_view($search_term); 
       $total_rows=$user->search_count_view($search_term);
       if($total_rows > 0){
           while ($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)){
        
                    extract($row2);
             echo '   <div class="column">
     <article class="card view_object" view_id="'.$food_id.'" table_id="'.$table_id.'" data-toggle="tooltip" data-placement="top" title="View">
                      <div>
                      
                                  ';
     $user->food_id = $food_id;
     $user->table_id = $table_id;
      $total_rows2 = $user->badge_count();
       $total_rows3 = $user->badge_count_2();
      if($total_rows2 > 0){
        echo'<h5><span class="badge badge-danger">'.$total_rows3.'</span></h5>';
      }
